...
class ADBannerView extends Component {
  ...
  componentDidMount() {
    this.subscriptionBannerView = NativeAppEventEmitter.addListener(
      "ADBannerView",
      (response) => this.handleBannerEvent(response)
    );
  }
  ...
  handleBannerEvent(event) {
    ...
    if (typeof this.props[event.eventName] === "function") {
      ...
      console.warn(
        `react-native-adbannerview: No property provided to handle event * ${event.eventName}*`
      );
    }
  }
  ...
  render() {
    ...
    return (
      <NativeADBannerView style={style} onLayout={this.handleLayoutChange} />
    );
  }
}

const NativeADBannerView = requireNativeComponent("ADBannerView", null);

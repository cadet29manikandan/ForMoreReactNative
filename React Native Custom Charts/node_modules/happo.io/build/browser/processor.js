...
var _findAssetPaths = _interopRequireDefault(require("../findAssetPaths"));

var _getComponentNameFromFileName = _interopRequireDefault(require("../getComponentNameFromFileName"));

var _getRenderFunc = _interopRequireDefault(require("./getRenderFunc"));

var _validateAndFilterExamples = _interopRequireDefault(require("./validateAndFilterExamples"));
...
function findRoot() {
  const {
    children
  } = document.body; // Find nodes that have some inner content and aren't script nodes.

  const layoutChildren = Array.from(children).filter(node => node.tagName.toLowerCase() !== 'script' && node.innerHTML !== '');
...
async function renderExample(exampleRenderFunc, {
  ...
  const renderInDom = renderResult => window.happoRender(renderResult, {
    rootElement,
    component,
    variant
  });
...
class Processor {
  constructor({
    ...
    this.onlyComponent = only ? only.split('#')[1] : undefined; // Array containing something like
    // [
    //    {
    //       fileName: '/foo/bar.js',
    //       component: 'Bar',
  ...
  init({
    ...
    this.flattenedExamples = (0, _validateAndFilterExamples.default)(this.flattenedUnfilteredExamples, {
      targetName
    });
  }
  ...
  prepare(fileName, exportsFromFile) {
    ...
    if (Array.isArray(exportsFromFile)) {
      ...
      this.addExamples(exportsFromFile.map(obj => Object.assign({
        fileName
      }, obj)));
    } else {
      const component = (0, _getComponentNameFromFileName.default)(fileName);
      window.verbose(`Found ${Object.keys(exportsFromFile).length} variant(s) for component ${component} in ${fileName}`);
      this.addExamples([{
        fileName,
        component,
        variants: exportsFromFile
      }]);
    }
  }
  ...
  async processCurrent() {
    ...
    const exampleRenderFunc = (0, _getRenderFunc.default)(render);
    window.happoCleanup();
    ...
    try {
      ...
      return new _WrappedError.default(`Failed to render component "${component}", variant "${variant}" in ${fileName}`, e);
    }

    const root = this.rootElementSelector && document.body.querySelector(this.rootElementSelector) || findRoot();
    ...
    const item = {
      ...
      component,
      variant,
      assetPaths: (0, _findAssetPaths.default)()
    };
    const {
      stylesheets
    } = render;
  ...
  extractCSS() {
    ...
    return styleElements.map(el => el.innerHTML || Array.from(el.sheet.cssRules).map(r => r.cssText).join('\n')).join('\n');
  }
  ...
  waitForHTML(elem, start = new Date().getTime(), attempt = 0) {
    ...
    if (html === '' && duration < this.asyncTimeout) {
      return new Promise(resolve => setTimeout(() => resolve(this.waitForHTML(elem, start, attempt + 1)), 10));
    }

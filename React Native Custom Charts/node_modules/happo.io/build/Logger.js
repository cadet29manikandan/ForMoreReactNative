...
function withColorSupport(wrappedFunc) {
  if (_supportsColor.default.stdout && _supportsColor.default.stderr) {
    return wrappedFunc;
  }
...
class Logger {
  ...
  start(msg, {
    ...
    this.startTime = startTime || _perf_hooks.performance.now();
  ...
  printDuration() {
    if (this.startTime) {
      this.print(dim(` (${(_perf_hooks.performance.now() - this.startTime).toFixed(1)}ms)`));
    }
  }
